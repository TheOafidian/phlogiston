...
def main(args=None):
    ...
    chart_parser.add_argument(
        "--output", "-o", default="/", help="Name of file to save output map to."
    )
    chart_parser.add_argument(
        "--extension",
        "-e",
        ...
        default="html",
        help="Extension to save output as.",
    )
    chart_parser.add_argument(
        "--dimensions",
        "-n",
        ...
        type=int,
        choices=[2, 3],
        help="Amount of dimensions to plot.",
    )
    chart_parser.add_argument(
        "--radius",
        "-r",
        default=0.2,
        type=float,
        help="Relative distance two spheres can maximally have to draw routes between them (0-1).",
    )
    chart_parser.add_argument(
        "--distance_metric",
        "-p",
        default=2,
        type=float,
        help="Minkowski distance metric to use.",
    )
    chart_parser.add_argument(
        "--name",
        "-s",
        default="",
        type=str,
        help="Name of the starchart. Gets added to the top of the figure.",
    )
    chart_parser.add_argument(
        "--dtime",
        "-t",
        default=1,
        type=float,
        help="Modifier to the distance in time betweeen different spheres.",
    )
    ...
    def relative_float(arg):
        ...
        if f < 0 or f > 1:
            raise argparse.ArgumentTypeError(
                "Relative float should be between 0 and 1."
            )
        return f
    ...
    random_parser.add_argument(
        ...
        default=100,
        type=int,
        help="The amount of spheres to populate the chart with.",
    )
    random_parser.add_argument(
        "--max-length",
        "-l",
        default=0.125,
        type=relative_float,
        help="The max distance between two spheres that is still counted as 'connected'. Relative between 0-1.",
    )
    random_parser.add_argument(
        "--seed",
        "-s",
        default=42,
        type=int,
        help="A seed for reproducible random charting.",
    )
